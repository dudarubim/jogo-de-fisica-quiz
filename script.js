...
function showResult() {
  ...
  // Verifica se o jogador tem erros e mostra o botão para mostrar erros
  if (errosDoJogador.length > 0) {
    btnMostrarErros.classList.remove("hidden");
    
    // Abre uma nova página (mostrarerros.html) ao clicar no botão
    btnMostrarErros.onclick = () => {
      window.open('mostrarerros.html', '_blank'); // Abre em uma nova aba
    };
  } else {
  ...
  // Verifica a pontuação do jogador e exibe a mensagem de resultado
  if (score === questions.length) {
    ...
    startConfetti(); // 🎊 Inicia os confetes se o jogador acertar tudo
  } else {
  ...
  // Atualiza o placar no armazenamento local
